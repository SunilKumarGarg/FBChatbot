...
class DeterministicGenerator:
    ...
    def combineSets(self, listOfSets):
        """
    ...
    def processSequence(self, seq):
        ...
        for component in seq:
            componentSets.append(self.processRHS(component))
        return self.combineSets(componentSets)


    def processNonTerminal(self, nt):
        """
        Finds the rule expansion for a nonterminal and returns its expansion.
        """
        return self.processRHS(self.grammar.getRHS(nt))

    def processDisjunction(self, disj):
        """
        ...
        for disjunct in disjuncts:
            disjunctExpansions.extend(self.processRHS(disjunct))
        return disjunctExpansions

    def processOptional(self, opt):
        """
        ...
        """
        optional = ['']
        optional.extend(self.processRHS(opt.option))
        return optional

    def processRHS(self, rhs):
        """
        ...
        """
        if type(rhs) is list:
            return self.processSequence(rhs)
        elif isinstance(rhs, gram.Disjunction):
            return self.processDisjunction(rhs)
        elif isinstance(rhs, gram.Optional):
            return self.processOptional(rhs)
        elif isinstance(rhs, gram.NonTerminal):
            return self.processNonTerminal(rhs)
